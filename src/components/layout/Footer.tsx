...
export function Footer() {
  ...
  return (
    <Box
      ...
      sx={{ bgcolor: 'background.paper', borderTop: '1px solid', borderColor: 'divider', py: { xs: 8, md: 11 } }}
    >
      <Container maxWidth="xl" sx={{ px: { xs: 'var(--wc-page-gutter-xs)', md: 'var(--wc-page-gutter-md)', xl: 'var(--wc-page-gutter-xl)' } }}>
        <Grid container spacing={{ xs: 5, md: 7 }} sx={{ mb: { xs: 7, md: 9 } }}>
          ...
          {footerColumns.map((column) => (
            <Grid key={column.title} size={{ xs: 12, sm: 6, md: 2.25 }} component={motion.div} initial={reduceMotion ? false : { opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.16 }} transition={{ duration: 0.45, delay: 0.06 }}>
              <Typography sx={{ fontWeight: 600, mb: 2.5, fontSize: 14 }}>{column.title}</Typography>
              <Stack spacing={1.6}>
        ...
        <Box component={motion.div} initial={reduceMotion ? false : { opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.5, delay: 0.1 }} sx={{ borderTop: '1px solid', borderColor: 'divider', pt: 4, display: 'flex', flexWrap: 'wrap', gap: 3, justifyContent: 'space-between' }}>
          <Box>
